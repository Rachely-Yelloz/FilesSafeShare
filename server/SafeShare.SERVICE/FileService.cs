...
namespace SafeShare.SERVICE
{
    public class FileService : IFileService
    {
        ...
        public async Task<int> UploadFileAsync(string pathInS3, string fileName, int userId, byte[] EncryptionKey, byte[] Nonce, string fileType)
        {
            return await _fileRepository.UploadFileAsync(pathInS3, fileName,userId, EncryptionKey,Nonce,  fileType);
        }
    }
}
